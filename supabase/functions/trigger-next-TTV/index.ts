// trigger-next-TTV/index.ts
// Queues the next pending TTV_task and fires process-TTV.
// Mirrors trigger-next-image exactly but for TTV_tasks.
...
async function queueNextBatch(
  ...
  try {
    ...
    // compileFinalTTVDocument is called directly from completeTask (process-TTV)
    // when the last batch finishes. We must NOT fire process-TTV here again —
    // doing so creates an infinite loop: process-TTV finds no actionable task,
    // calls triggerNextTTV, which fires process-TTV again, ad infinitum.
    ...
    // Fire-and-forget: invoke process-TTV for the next batch
    fetch(`${supabaseUrl}/functions/v1/process-TTV`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      logError(`Error initiating process-TTV for batch ${nextTask.batch_number}`, err);
      supabase
        .from('TTV_tasks')
        .update({
          status: 'running',
          error: `Failed to trigger process-TTV: ${err.message}`,
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (elapsed > maxRuntime) console.warn(`trigger-next-TTV runtime exceeded safe limit: ${elapsed}ms`);

    return new Response(JSON.stringify(result), { status: 200, headers: responseHeaders });
  } catch (error: any) {
    await logError('Error in trigger-next-TTV', error);
    return new Response(
