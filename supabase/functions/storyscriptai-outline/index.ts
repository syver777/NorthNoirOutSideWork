...
// Self-retry: when the outline call fails (timeout, hung isolate, AI error,
// parse failure, anything) the handler fires a fresh HTTP POST to itself with
// _retryAttempt incremented. After MAX_SELF_RETRIES total attempts (0..4) the
// 5th invocation logs and exits silently without writing an error status to
// the database. No error_message is ever written by this retry path. Kept in
// lock-step with denodeploy/storyscriptai-outline.ts so the Supabase fallback
// behaves identically to the primary Deno Deploy worker.
const MAX_SELF_RETRIES = 5;
const SELF_FUNCTION_URL = Deno.env.get('OUTLINE_FUNCTION_URL')
  || `${Deno.env.get('SUPABASE_URL') || ''}/functions/v1/storyscriptai-outline`;
...
const MODEL_CONFIGS = {
  ...
  sonnet: {
    ...
    // well before the edge isolate is recycled (otherwise the task gets stuck
    // in `processing` forever with no log line after "Attempt 1").
    timeout: 180000
  },
  opus: {
    maxWordsPerBatch: 3000, // Updated from 3500 to 3000
...
// Detect content type using a fast AI classification call
...
// Build content-type-specific instructions to append to outline system prompts
...
async function triggerParseFunction(
  ...
  while (retries < PARSE_RETRY_ATTEMPTS) {
    try {
      ...
      const response = await fetchWithDenoFallback('storyscriptai-parse', {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "apikey": SUPABASE_SERVICE_ROLE_KEY,
...
async function generateOutline(
  ...
  // Detect content type using AI classification
  // If master-prompt already detected it (stored on pre-inserted row), reuse that
  ...
  if (existingOutlineTaskId) {
    try {
      const { data: existingRow } = await supabase
        .from('story_tasks')
        .select('content_type')
        .eq('id', existingOutlineTaskId)
        .single();
      if (existingRow?.content_type && existingRow.content_type !== 'story') {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (_authToken === SUPABASE_SERVICE_ROLE_KEY || _authToken === SUPABASE_SECRET_KEY) {
      // Service call or backward-compatible frontend call
      // TODO: Remove SUPABASE_ANON_KEY check after frontend deploys with JWT auth
    } else {
