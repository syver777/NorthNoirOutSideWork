...
` : ''}`;
  ...
  const prompts: Record<string, string> = {
    english: `You are an expert cinematic director. Generate concise 150-250 word video prompts for each text segment.

CRITICAL: Create exactly ONE video prompt per text segment. Do NOT split or merge segments.

For each segment, write a single flowing cinematic description in this order:
1. Narrative action: what is happening, who is present, what they are doing — the specific story moment being depicted
2. Camera framing and movement (e.g. slow push in on a medium close-up, wide tracking shot)
3. Lighting and atmosphere (e.g. harsh overhead fluorescents, golden-hour side lighting)
4. Style: ${effectiveStyle}${visualGuidelinesBlock}
${audioClip ? `5. Audio atmosphere: weave a natural description of ambient sound and sound design directly into the paragraph. Scale to clip duration: 4s → one precise sound cue; 6–8s → 2–3 layered sounds; 10s+ → an evolving soundscape. Do NOT use labels like "Audio:" or "Sound Design:" — it must read as part of the cinematic prose.
` : ''}- Character body language and emotional expression — refer to characters by name only
${effectiveEnvironmentOnly ? '- ENVIRONMENT-ONLY MODE: Focus on the location and atmosphere as the main subject. Minimal character focus — do not show character faces in detail.' : ''}
${charSystemInstruction}
${charReferenceBlock}
RULES:
- Write ONE cohesive paragraph per prompt. No sub-headings, no labeled sections.
- No 'Cinematography:', 'Audio Design:', 'Lighting:', 'Transition Note:', or any similar labels.
- Keep language visual and motion-oriented.
${audioClip ? `- Audio atmosphere is described in-line within the paragraph — never as a separate section or with any label.\n` : ''}- ⚠️ CRITICAL: The video must NOT contain any visible text, letters, words, numbers, signs, or written language.

Output a JSON array with exactly the same number of items as input segments.
Each item must have keys: "text" (exact copy of the input segment text), "prompt" (your 150-250 word cinematic description)${schemaInstruction}.
...
serve(async (req: Request) => {
  ...
  try {
    ...
    let userPrompt = `Process these ${normalizedSegments.length} segment(s). For EACH segment, copy the "text" field EXACTLY and generate ONE cinematic video prompt:\n\n`;
    normalizedSegments.forEach((seg, idx) => {
      userPrompt += `Segment ${idx + 1} (${seg.text.length} chars, ${seg.video_duration}s video):\nText: ${seg.text}\n\n`;
    });
...
Return EXACTLY ${normalizedSegments.length} result(s) in the JSON array.`;
    ...
    if (elapsed > maxRuntime) console.warn(`generate-TTV-prompt runtime exceeded safe limit: ${elapsed}ms`);
    ...
    console.error(`Error in generate-TTV-prompt: ${error.message}`);
    await logError('Error in generate-TTV-prompt', error);
