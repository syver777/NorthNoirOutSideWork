...
async function queueNextBatch(groupId: string, userId: string, currentBatchNumber: number, tab: number = 1, variant: number = 1): Promise<{ message: string; batch_number: number }> {
  try {
    console.log(`Fetching tasks for group ${groupId}, user ${userId}, tab ${tab}, variant ${variant}`);
    ...
    if (tasksError) {
      const errorMsg = `Failed to fetch tasks: ${tasksError.message}`;
      console.error(errorMsg);
      throw new Error(errorMsg);
    }
    console.log(`Current story_tasks state: ${JSON.stringify(tasks, null, 2)}`);
    ...
    if (currentBatchNumber >= totalBatches && totalBatches > 0) {
      console.log('All batches processed or current batch is last. Triggering final compilation.');
      const { data: task } = await supabase
        ...
        .single();
      if (task) {
        await fetch(`${supabaseUrl}/functions/v1/process-story`, {
          method: 'POST',
          headers: {
          ...
          logError(`Error triggering final compilation`, error);
        });
      }
      return { message: 'All batches completed, final compilation triggered', batch_number: currentBatchNumber };
    }
    ...
    if (!incompleteBatch) {
      ...
      if (runningBatch) {
        console.log(`Batch ${runningBatch.batch_number} is already running ahead of current batch ${currentBatchNumber}`);
        return { message: `Batch ${runningBatch.batch_number} already running`, batch_number: runningBatch.batch_number };
      }
      
      const errorMsg = `No incomplete tasks found after batch ${currentBatchNumber}`;
      console.error(errorMsg);
      throw new Error(errorMsg);
    }

    console.log(`Found incomplete batch ${incompleteBatch.batch_number} with status ${incompleteBatch.status} for group ${groupId}`);
    ...
    if (incompleteBatch.batch_number > currentBatchNumber + 1) {
      console.warn(`Sequence gap detected: current batch ${currentBatchNumber}, next incomplete batch ${incompleteBatch.batch_number}`);
      ...
      if (runningInBetween) {
        console.log(`Batch ${runningInBetween.batch_number} is running between current and next incomplete batch`);
        return { message: `Batch ${runningInBetween.batch_number} already running`, batch_number: runningInBetween.batch_number };
      }
    }
    
    // Only update status if it's not already queued
    if (incompleteBatch.status !== 'queued') {
      console.log(`Queueing batch ${incompleteBatch.batch_number} for group ${groupId}`);
      const { error: updateError } = await supabase
        .from('story_tasks')
        .update({ status: 'queued', updated_at: new Date().toISOString() })
        .eq('id', incompleteBatch.id);
      if (updateError) {
        const errorMsg = `Failed to queue batch ${incompleteBatch.batch_number}: ${updateError.message}`;
        console.error(errorMsg);
        throw new Error(errorMsg);
      }
    } else {
      console.log(`Batch ${incompleteBatch.batch_number} is already queued for group ${groupId}`);
    }
    
    // Trigger process-story asynchronously via fetch
    console.log(`Initiating process-story for batch ${incompleteBatch.batch_number}`);
    fetch(`${supabaseUrl}/functions/v1/process-story`, {
      ...
      console.error(`Error initiating process-story for batch ${incompleteBatch.batch_number}: ${error.message}`);
      logError(`Error initiating process-story for batch ${incompleteBatch.batch_number}`, error);
    });
    return { message: `Started batch ${incompleteBatch.batch_number} processing`, batch_number: incompleteBatch.batch_number };
  } catch (error: any) {
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.error(`Error in trigger-next-batch: ${error.message}\nStack: ${error.stack}`);
    await logError('Error in trigger-next-batch', error);
