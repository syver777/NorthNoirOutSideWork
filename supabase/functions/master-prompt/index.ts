...
const SUPABASE_ANON_KEY = (Deno.env.get('PUBLIC_KEY'));
...
Provide ONLY the JSON object, no additional text.`;
  ...
  try {
    ...
    const AI_ENHANCEMENT_TIMEOUT = 120000; // 120 seconds — must finish before Deno Deploy kills the isolate
    ...
    if (model === 'deepseek') {
      const response = await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error('AI enhancement timeout (DeepSeek)')), AI_ENHANCEMENT_TIMEOUT)),
      ]) as any;
      ...
      const response = await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error(`AI enhancement timeout (${model})`)), AI_ENHANCEMENT_TIMEOUT)),
      ]) as any;
...
async function triggerOutlineGenerationAsync(
  ...
  try {
    ...
    const response = await fetchWithDenoFallback('storyscriptai-outline', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
Deno.serve(async (req: Request) => {
  ...
  // Hoist these so they're accessible in the catch block for cleanup
  let _cleanup_user_id: string | null = null;
  let _cleanup_group_id: string | null = null;
  let _cleanup_tab: number = 1;
  ...
  try {
    ...
    // Store for cleanup in catch block
    _cleanup_user_id = user_id;
    _cleanup_group_id = group_id;
    _cleanup_tab = tab;

    // Detect content type early so we can store it on the pre-insert row
    // Uses title + description + master_prompt_data + pre-fetched transcript if available
    const contentType = await detectContentType(
      title,
      description,
      master_prompt_data,
      (typeof youtube_transcript_text === 'string' && youtube_transcript_text.length > 0) ? youtube_transcript_text : undefined
    );
    ...
    // If a row already exists for (group_id, user_id, batch, version) — which can happen when the
    // Deno Deploy primary stalled and we're handling the retry — reuse that row's id and update
    // it in place so we don't break any downstream FK references and don't 409.
    ...
    try {
      const preInsertResponse = await fetch(`${SUPABASE_URL}/rest/v1/story_tasks`, {
        ...
        body: JSON.stringify(preInsertBody),
      });
      if (preInsertResponse.ok) {
        console.log(`Pre-inserted processing task ${outlineTaskId} for group ${group_id}, tab ${tab}`);
      } else {
        const errText = await preInsertResponse.text().catch(() => '');
        const isUniqueViolation = preInsertResponse.status === 409 || errText.includes('23505');
        if (isUniqueViolation) {
          ...
          // Uses the same filter shape as the cleanup query elsewhere in this flow — the
          // pre-insert row is uniquely identified by (group_id, user_id, tab, batch_number=0).
          const existingResp = await fetch(
            `${SUPABASE_URL}/rest/v1/story_tasks?group_id=eq.${group_id}&user_id=eq.${user_id}&tab=eq.${tab}&batch_number=eq.0&select=id&order=created_at.desc&limit=1`,
            {
              method: 'GET',
              headers: {
                'apikey': SUPABASE_SERVICE_ROLE_KEY,
          ...
          if (existingResp.ok) {
            const rows = await existingResp.json().catch(() => []);
            ...
            if (existingId) {
              ...
              const updateResp = await fetch(
                `${SUPABASE_URL}/rest/v1/story_tasks?id=eq.${existingId}`,
                {
                  method: 'PATCH',
                  headers: {
                    'Content-Type': 'application/json',
              ...
              if (updateResp.ok) {
                ...
                const updErr = await updateResp.text().catch(() => '');
                console.warn(`Failed to refresh existing processing task ${existingId}: HTTP ${updateResp.status} - ${updErr}`);
              }
            } else {
              console.warn(`Pre-insert hit unique violation but could not locate existing row for group ${group_id}`);
            }
          } else {
            console.warn(`Pre-insert hit unique violation; lookup failed: HTTP ${existingResp.status}`);
          }
        } else {
          console.warn(`Failed to pre-insert processing task: HTTP ${preInsertResponse.status} - ${errText}`);
        }
      }
    ...
    // Build combined raw transcript text for factual context in story generation
    ...
    // Clean up the pre-inserted processing row so it doesn't orphan and block
    // future outline generation attempts.
    if (_cleanup_group_id && _cleanup_user_id) {
      try {
        console.log(`Cleaning up orphaned processing row for group ${_cleanup_group_id}, tab ${_cleanup_tab}`);
        await fetch(
          `${SUPABASE_URL}/rest/v1/story_tasks?group_id=eq.${_cleanup_group_id}&user_id=eq.${_cleanup_user_id}&tab=eq.${_cleanup_tab}&batch_number=eq.0&status=eq.processing&outline=is.null`,
          {
            method: 'DELETE',
            headers: {
              'apikey': SUPABASE_SERVICE_ROLE_KEY,
        ...
        console.warn(`Failed to clean up orphaned row: ${cleanupErr.message}`);
      }
    }
