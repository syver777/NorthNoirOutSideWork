...
function cleanTextForRF(text: string): string {
...
function forceExactSegments(text: string, n: number, videoDuration: number): RFSegment[] {
  if (n <= 0) return [];
...
async function insertRFTasks(tasks: any[], startTime: number, maxRuntime: number) {
  ...
  for (let i = 0; i < total; i += BATCH_SIZE * 10) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        const { error } = await supabase.from('RF_prompt_tasks').insert(batch);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (videoDurationClamped) {
      console.warn(`[setup-RF-prompts] video_duration ${parsedVideoDuration}s out of range for model "${video_model}" — clamped to ${clampedVideoDuration}s`);
    }
    ...
    // Check both RF_prompt_tasks (prompt generation) and RF_tasks (video generation)
    ...
    if (promptTasksRes.error) {
      console.warn(`Warning: Could not check RF_prompt_tasks variants: ${promptTasksRes.error.message}`);
    }
    if (videoTasksRes.error) {
      console.warn(`Warning: Could not check RF_tasks variants: ${videoTasksRes.error.message}`);
    }
    ...
    const cleanedText = cleanTextForRF(rawContent);
    ...
    const genEstimate = estimateRFTokens(wordCount, total_videos, useCharacterDescriptions, validatedModel, isLegacyPlan);
    ...
    // ── Trigger process-RF-task ──────────────────────────────────────────────

    try {
      const resp = await fetchSupabaseFunction('process-RF-tasks', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      console.log(`Triggered process-RF-task for jobId=${jobId}`);
    } catch (triggerErr: any) {
      await supabase.from('job_data').delete().eq('id', jobId);
      throw new Error(`Failed to trigger process-RF-task: ${triggerErr.message}`);
    }
