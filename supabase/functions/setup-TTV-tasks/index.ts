// setup-TTV-tasks/index.ts
// Sets up TTV_tasks rows from a compiled TTV prompts JSON file (version 12/13 → 14/15)
...
const SUPPORTED_VIDEO_MODELS = [
  'seedance_pro_fast', 'ltx23_fast', 'seedance15_pro', 'ltx23_pro',
  'grok', 'grok_highres',
  'veo31fast', 'veo31', 'sora2pro', 'sora2pro_highres',
];
...
async function insertTasksInBatches(tasks: any[], startTime: number, maxRuntime: number) {
  ...
  for (let i = 0; i < tasks.length; i += CHUNK) {
    ...
    const { error } = await supabase.from('TTV_tasks').insert(slice);
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!video_model || typeof video_model !== 'string')
      return new Response(JSON.stringify({ error: 'Missing video_model' }), { status: 400, headers: responseHeaders });
    if (!SUPPORTED_VIDEO_MODELS.includes(video_model))
    ...
    const [existingTasksRes, existingDocsRes] = await Promise.all([
      supabase.from('TTV_tasks').select('variant')
    ...
    // Check if a video_tasks row exists with visual_type='ttv' for this group.
    // If yes, these TTV_tasks are part of the video pipeline and need video_process=true
    ...
    try {
      const { data: vtRow } = await supabase
        ...
        .maybeSingle();
      videoProcess = !!vtRow;
      if (videoProcess) console.log('Video pipeline detected — will set video_process=true on TTV_tasks');
    } catch (err: any) {
    ...
    fetch(`${supabaseUrl}/functions/v1/trigger-next-TTV`, {
      method: 'POST',
      headers: {
    ...
    await logError('Error in setup-TTV-tasks', error);
    return new Response(
