// trigger-next-ITV-prompt/index.ts
// Queues the next unprocessed ITV_prompt_tasks batch and fires process-ITV-prompt.
// The `itv` flag in the request body selects Phase 1 (false) or Phase 2 (true).
...
async function queueNextBatch(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (error) {
      ...
      throw new Error(`Failed to fetch ITV prompt tasks: ${error.message}`);
    }

    tasks = data ?? [];
    console.log(`Fetched ${tasks.length} ITV_prompt_tasks (itv=${itvFlag}) for group ${groupId}, tab ${tab}, variant ${variant}`);
    if (tasks.length > 0) break;
    console.log('No tasks found yet, retrying in 2s…');
    await new Promise(r => setTimeout(r, 2000));
  }

  if (tasks.length === 0) throw new Error(`No ITV prompt tasks found for group ${groupId}, itv=${itvFlag}`);
  ...
  if (completedTasks.length >= totalBatches && totalBatches > 0) {
    console.log(`All ITV prompt batches (itv=${itvFlag}) completed.`);
  ...
  if (!nextTask) {
    ...
    throw new Error(`No queued/pending/error ITV prompt task found for batch >= ${currentBatchNumber}, itv=${itvFlag}`);
  }

  console.log(`Selected ITV prompt task: batch ${nextTask.batch_number}, status ${nextTask.status}, id ${nextTask.id}, itv=${itvFlag}`);
  ...
  if (nextTask.status !== 'queued') {
    const { error: updateError } = await supabase
      ...
      .eq('id', nextTask.id);
    if (updateError) throw new Error(`Failed to re-queue ITV prompt batch ${nextTask.batch_number}: ${updateError.message}`);
  }

  console.log(`Fire-and-forget: triggering process-ITV-prompt for batch ${nextTask.batch_number} (itv=${itvFlag})`);
  fetch(`${supabaseUrl}/functions/v1/process-ITV-prompt`, {
    method: 'POST',
    headers: {
    ...
    console.error(`Error firing process-ITV-prompt for batch ${nextTask.batch_number}: ${err.message}`);
    logError(`Error firing process-ITV-prompt for batch ${nextTask.batch_number} (itv=${itvFlag})`, err);
    const status = isRetryableError(err) ? 'running' : 'pending';
    supabase
      .from('ITV_prompt_tasks')
      .update({ status, error: `Failed to trigger process-ITV-prompt: ${err.message}`, updated_at: new Date().toISOString() })
      .eq('id', nextTask.id);
  });

  return { message: `Started ITV prompt batch ${nextTask.batch_number} processing (itv=${itvFlag})`, batch_number: nextTask.batch_number };
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`trigger-next-ITV-prompt received: ${JSON.stringify(payload)}`);
    ...
    const { group_id, user_id, current_batch_number, itv, tab = 1, variant = 1 } = payload;
    const result = await queueNextBatch(group_id, user_id, current_batch_number, itv, tab, variant);
    console.log(`ITV prompt batch ${result.batch_number} queued (itv=${itv}): ${result.message}`);

    return new Response(JSON.stringify(result), { status: 200, headers: responseHeaders });
  } catch (error: any) {
    console.error(`Error in trigger-next-ITV-prompt: ${error.message}`);
    await logError('Error in trigger-next-ITV-prompt', error);
