...
async function queueNextBatch(
  ...
  console.log(`Fire-and-forget: triggering process-RF-prompt for batch ${nextTask.batch_number}`);
  fetch(`${supabaseUrl}/functions/v1/process-RF-prompt`, {
    method: 'POST',
    headers: {
    ...
    console.error(`Error firing process-RF-prompt for batch ${nextTask.batch_number}: ${err.message}`);
    logError(`Error firing process-RF-prompt for batch ${nextTask.batch_number}`, err);
    const status = isRetryableError(err) ? 'running' : 'pending';
    supabase
      .from('RF_prompt_tasks')
      .update({ status, error: `Failed to trigger process-RF-prompt: ${err.message}`, updated_at: new Date().toISOString() })
      .eq('id', nextTask.id);
  });
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`trigger-next-RF-prompt received: ${JSON.stringify(payload)}`);
    ...
    console.error(`Error in trigger-next-RF-prompt: ${error.message}`);
    await logError('Error in trigger-next-RF-prompt', error);
