...
async function triggerTTVVideoGeneration(
  ...
  try {
    ...
    const response = await fetch(`${supabaseUrl}/functions/v1/setup-TTV-tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'apikey': supabaseServiceRoleKey,
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to trigger setup-TTV-tasks: HTTP ${response.status}: ${errorText.slice(0, 300)}`);
    }

    console.log(`Successfully triggered setup-TTV-tasks for group ${groupId}, tab ${tab}`);
  } catch (err: any) {
  ...
  for (let attempt = 0; attempt <= retryDelays.length; attempt++) {
    try {
      ...
      if (currentBatchNumber >= totalBatches) {
        ...
        const { documentId, filePath, videoDuration } = await compileFinalTTVDocument(
      ...
      console.log(`Triggering trigger-next-TTV-prompt for batch ${nextBatchNumber}, group ${groupId}, tab ${tab}`);
...
// ─── Call generate-TTV-prompt ─────────────────────────────────────────────────

async function callGenerateTTVPrompts(payload: any, taskId: string, batchNumber: number): Promise<any> {
  const retryDelays = [10000, 20000, 40000, 80000, 160000, 320000];
  for (let attempt = 0; attempt <= retryDelays.length; attempt++) {
    try {
      console.log(`Calling generate-TTV-prompt for batch ${batchNumber}, attempt ${attempt + 1}`);
      ...
      try {
        response = await fetch(`${supabaseUrl}/functions/v1/generate-TTV-prompt`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'apikey': supabaseServiceRoleKey,},
...
async function processTTVTask(
  ...
  try {
    ...
    const chunkResult = await callGenerateTTVPrompts(payload, task.id, batchNumber);
    ...
    console.error(`Error in processTTVTask batch ${batchNumber}: ${err.message}`);
    await logError('Error in processTTVTask', err);
    await supabase.from('TTV_prompt_tasks')
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`process-TTV-prompt: batch ${batch_number}, group ${group_id}, tab ${tab}, variant ${variant}`);

    await resetStuckTasks(group_id, user_id, tab, variant);

    let task: TTVTask | null = null;
    for (let attempt = 0; attempt < 3; attempt++) {
      const { data, error } = await supabase
        .from('TTV_prompt_tasks')
        .select('*')
    ...
    const processPromise = processTTVTask(task, group_id, user_id, batch_number, total_batches, tab, variant)
      .catch(async err => {
    ...
    responseSent.value = true;
    return new Response(
      JSON.stringify({ message: 'TTV processing started, results will be available in TTV_prompt_tasks', batch_number }),
      { status: 202, headers: responseHeaders },
    );
  } catch (error: any) {
    console.error(`Error in process-TTV-prompt: ${error.message}`);
    await logError('Error in process-TTV-prompt', error);
    if (payload) {
      await supabase.from('TTV_prompt_tasks')
