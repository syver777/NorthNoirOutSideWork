...
import { estimateVideoPipelineSeconds, TOKEN_RATES_PER_MIN } from '../_shared/timeEstimates.ts';
import { estimateMgTokenCost, mgClipCount, MG_DEFAULT_CLIP_SECONDS } from '../_shared/mgCosts.ts';
...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !SUPABASE_SERVICE_ROLE_KEY) {
  throw new Error("Missing SUPABASE_URL, PUBLIC_KEY, or SECRET_KEY");
}
if (!DEEPSEEK_API_KEY) {
...
const IMAGE_MODEL_COSTS: Record<string, { tokens: number; name: string; tier: string }> = {
  ...
  'gpt-image-1-mini':   { tokens: 30000,  name: 'Core',     tier: 'plus' },
  'seedream-4.5':       { tokens: 35000,  name: 'Prime',    tier: 'prime' },
  'imagen-4-ultra':     { tokens: 42000,  name: 'Heavy',    tier: 'premium' },
...
const NEW_IMAGE_MODEL_COSTS: Record<string, { tokens: number; name: string; tier: string }> = {
  ...
  'gpt-image-1-mini':   { tokens: 40000,  name: 'Core',     tier: 'plus' },
  'seedream-4.5':       { tokens: 40000,  name: 'Prime',    tier: 'prime' },
  'imagen-4-ultra':     { tokens: 60000,  name: 'Heavy',    tier: 'premium' },
...
// Mirrors LEGACY_LLM_MULTIPLIERS in supabase/functions/_shared/tokenCosts.ts.
const STORY_MODEL_MULTIPLIERS: Record<string, number> = {
  deepseek: 1.0,
  sonnet: 11.0,
};
// New plan pricing (is_legacy_plan = false).
...
function estimateTokenCost(
  ...
  // Use the master prompt's character flag to estimate prompt-batch overhead
  // identically to video-analyze (so plan-video stops under-quoting sonnet).
  ...
  if (settings.visual_type === 'image' && settings.image_model) {
    ...
    if (imageModelCost) {
      // Estimate image count from frequency
      ...
      // Image prompt generation — uses video-analyze's calibrated formula
      // (numBatches × (wordCount + 1600) × tokenPerWord), which models the
      // full per-batch context overhead instead of a flat 200-tok constant.
      ...
      // Pipeline overhead — runtime-based via shared timeEstimates module
    ...
    if (model) {
      ...
      // TTV prompt generation — same calibrated formula
    ...
    if (model) {
      ...
      const clipCount = Math.ceil(runtimeSeconds / clipDuration);
      // ITV needs images first + video prompts
      totalTokens += estimateImagePromptTokens(wordCount, clipCount, useChars, promptMultiplier);
      // Image generation for keyframes
    ...
    // Clip duration is fixed at MG_DEFAULT_CLIP_SECONDS for now.
    ...
    // MG prompt-doc generation reuses the calibrated batch formula so the
    // narration → per-clip segment LLM call is properly budgeted.
    totalTokens += estimateImagePromptTokens(wordCount, clipCount, useChars, promptMultiplier);
    // Codegen + render (token-equivalent USD-based estimate).
    totalTokens += estimateMgTokenCost(runtimeSeconds, clipDuration);
    // Pipeline overhead — treat like TTV for stitch/audio/transitions.
    totalTokens += estimatePipelineTokens({
      numClips: clipCount,
      audioSeconds,
      visualType: 'ttv',
...
// and the page-level estimator agree on cost. Models the per-batch context
// overhead (system prompt + previous prompts + chapter + characters).
...
// Replaces the old PIPELINE_COSTS hardcoded constants which under-modelled
// long videos and over-modelled short ones.
function estimatePipelineTokens(args: {
  ...
  const breakdown = estimateVideoPipelineSeconds({
    N_images: numClips,
    S_audio: audioSeconds,
    durations: null,
    visualType,
    hasTransitions,
    hasOverlay: !!(animationType || effectsType),
    hasSubtitles: false, // plan-video doesn't currently model subtitles
  ...
  // 1.15× safety pad to align with video-analyze (Cloud Run variance).
...
Respond with ONLY a JSON object, no other text:
{
  ...
  "animation_type": "horizontal_drift" | "none",
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (authToken === SUPABASE_SERVICE_ROLE_KEY || authToken === SUPABASE_SECRET_KEY) {
    ...
    const {
      ...
      video_task_id,
      subtitles, // Optional burn-in subtitle config (jsonb). NULL/undefined = no subtitles.
    } = _reqBody;
    ...
    // Default to legacy pricing if the column is missing/null — never overcharge.
    ...
    const setupPayload: Record<string, unknown> = {
      ...
      // Optional burn-in subtitles (passed straight through to setup-video-tasks)
      ...(subtitles ? { subtitles } : {}),
