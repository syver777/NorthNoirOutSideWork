...
async function insertRFTasksInChunks(tasks: RFTask[], startTime: number, maxRuntime: number) {
  ...
  for (let i = 0; i < tasks.length; i += TASK_INSERT_CHUNK) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        const { error } = await supabase.from('RF_prompt_tasks').insert(chunk);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Fetch all text parts from RF_prompt_context ──────────────────────────
    ...
    await insertRFTasksInChunks(allTasks, startTime, maxRuntime);
    ...
    console.error(`Error in process-RF-task: ${error.message}`);
