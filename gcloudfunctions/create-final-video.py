...
_GCF_SUFFIX = ''  # gc_version=1 (base file)
...
def estimate_final_video_size(loop_video_path=None, target_duration=None,
                              local_video_files=None, audio_path=None):
    """Estimate final video size in GB"""
    try:
        estimated_size = 0

        if loop_video_path and target_duration and os.path.exists(loop_video_path):
            # Loop video case
            loop_size = os.path.getsize(loop_video_path)
            loop_duration = get_video_duration(loop_video_path)
            if loop_duration > 0:
                loops_needed = target_duration / loop_duration
                estimated_size = loop_size * loops_needed * 1.1  # 10% overhead
                print(
                    f"Loop video estimation: {loop_size / (1024**3):.2f} GB × {loops_needed:.2f} loops = {estimated_size / (1024**3):.2f} GB")
        elif local_video_files:
            # Individual videos case
            total_size = sum(os.path.getsize(f)
                             for f in local_video_files if os.path.exists(f))
            estimated_size = total_size * 1.2  # 20% overhead for concatenation
            print(
                f"Individual videos estimation: {total_size / (1024**3):.2f} GB × 1.2 overhead = {estimated_size / (1024**3):.2f} GB")

        # Add audio size
        if audio_path and os.path.exists(audio_path):
            audio_size = os.path.getsize(audio_path)
            estimated_size += audio_size * 0.1  # Audio adds ~10%
            print(
                f"Audio size consideration: +{(audio_size * 0.1) / (1024**3):.2f} GB")

        estimated_gb = estimated_size / (1024**3)  # Convert to GB
        print(f"Total estimated final video size: {estimated_gb:.2f} GB")
        return estimated_gb

    except Exception as e:
        print(f"Error estimating video size: {str(e)}")
        return 0


def delegate_to_high_memory(request_data):
    """Forward request to high-memory function (same gc_version as this instance)"""
    try:
        print("Delegating to high-memory function due to large estimated file size")
        url = (f"https://us-central1-story-script-ai.cloudfunctions.net/"
               f"create-final-video-high-memory{_GCF_SUFFIX}")

        service_role_key = (os.getenv("SUPABASE_SECRET_KEY") or "")
        ...
        if response.ok:
            print("Successfully delegated to high-memory function")
            return response.json()
        else:
            print(
                f"High-memory function returned error: {response.status_code} - {response.text}")
            return {"error": f"High-memory function failed: {response.status_code}"}

    except Exception as e:
        print(f"Error delegating to high-memory function: {str(e)}")
        return {"error": f"Failed to delegate to high-memory function: {str(e)}"}
...
def final_assembly_from_transition_batches(supabase, video_task_id, user_id, group_id, temp_dir):
    ...
    try:
        ...
        # Free /tmp space: delete individual batch files now that they are
        # merged into concatenated.mp4 (each ~200 MB × 23 = ~4.7 GB saved).
        for batch_file in local_batch_files:
            try:
                os.remove(batch_file)
            except Exception as _del_err:
                print(
                    f"Warning: could not delete batch file {batch_file}: {_del_err}")
        print(
            f"Deleted {len(local_batch_files)} batch files to free /tmp space")
...
# 20 min source per chunk (~6-10 min libx264 burn, safe under 30 min wall)
_SUBS_TARGET_CHUNK_SEC = 1200.0
...
def create_final_video(request):
    ...
    try:
        ...
        try:
            ...
            print(f"[metadata] create-final-video: {_meta_err}")
        ...
        if final_assembly_flag:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # SIZE CHECK: Estimate final video size before combining with audio.
                # Threshold is 4.5 GB because ffmpeg needs both the concatenated input
                # AND the final output on /tmp simultaneously (~2× the file size).
                # At 16 GiB RAM the /tmp budget is ~16 GB; 4.5 GB × 2 = 9 GB leaves
                # headroom for process memory and audio files.
                estimated_size = estimate_final_video_size(
                    local_video_files=[concatenated_video_path],
                    audio_path=audio_local_path
                )

                if estimated_size > 7.0:
                    print(
                        f"Estimated final video size ({estimated_size:.2f} GB) exceeds 7 GB limit, delegating to high-memory function")
                    return add_cors_headers(request, delegate_to_high_memory(data))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if video_loop_url:
                ...
                if len(individual_videos_paths) == 1 and individual_videos_paths[0] == video_loop_url:
                    ...
                    # SIZE CHECK: Estimate looped video size before creating it - THRESHOLD: 6.5 GB
                    estimated_size = estimate_final_video_size(
                    ...
                    if estimated_size > 7.0:
                        print(
                            f"Estimated looped video size ({estimated_size:.2f} GB) exceeds 7 GB limit, delegating to high-memory function")
                        return add_cors_headers(request, delegate_to_high_memory(data))
                    ...
                    if len(individual_videos_paths) == 1 and "video_loop" in individual_videos_paths[0]:
                        ...
                        # SIZE CHECK: Estimate size before processing - THRESHOLD: 6.5 GB
                        target_duration = loop_time or audio_duration + 10
                        estimated_size = estimate_final_video_size(
                            loop_video_path=loop_video_path,
                            target_duration=target_duration,
                            audio_path=audio_local_path
                        ...
                        if estimated_size > 7.0:
                            print(
                                f"Estimated video loop size ({estimated_size:.2f} GB) exceeds 7 GB limit, delegating to high-memory function")
                            return add_cors_headers(request, delegate_to_high_memory(data))
                ...
                # SIZE CHECK: Estimate final video size before concatenation - UPDATED TO 6 GB
                estimated_size = estimate_final_video_size(
                    local_video_files=local_video_files,
                    audio_path=audio_local_path
                )
                ...
                # Free ~size-of-inputs from /tmp now that concat.mp4 holds the
                # same bytes. On GCF /tmp is tmpfs (RAM-backed); keeping the
                # 73 source clips around while final_video.mp4 is being written
                # was pushing total tmpfs usage past the 16 GiB instance budget
                # and causing the bg-music mux to OOM.
                for _vf in local_video_files:
                    try:
                        if os.path.exists(_vf):
                            os.remove(_vf)
                    except OSError as _e:
                        print(f"Could not delete {_vf}: {_e}")
                local_video_files = []
