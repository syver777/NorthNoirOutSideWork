...
import ImageGenerator, { type ImageGeneratorRef } from './ImageGenerator';
import { checkIsEnterpriseUser, ensureTabExists, getUserActiveTabs, type TabInfo } from '../utils/tabManager';

/**
 * ImageGeneratorContainer - Wrapper component that manages tab state and forces
 * ImageGenerator component remounting when tabs change to ensure complete isolation
 */
export default function ImageGeneratorContainer() {
  ...
  const imageGeneratorRefs = useRef<Record<number, ImageGeneratorRef>>({});
  ...
  useEffect(() => {
    ...
    const initializeContainer = async () => {
      try {
        ...
        // Ensure Tab 1 exists in database for image generator page
        if (isEnterprise) {
          await ensureTabExists(user.id, 'image');
          const tabs = await getUserActiveTabs(user.id, 'image');
          setPrefetchedTabs(tabs);
        }

        setIsInitialized(true);
      } catch (error) {
        console.error('Error initializing image generator container:', error);
  ...
  const handleTabChange = (tab: number, groupId: string) => {
    console.log(`[ImageGeneratorContainer] Tab change requested: ${currentTab} -> ${tab}, groupId: ${groupId}`);
  ...
  const handleTabCreate = (tab: number, groupId: string) => {
    console.log(`[ImageGeneratorContainer] Tab create: ${tab}, groupId: ${groupId}`);
  ...
  const handleTabClose = async (tab: number, groupId: string) => {
    console.log(`[ImageGeneratorContainer] Tab close: ${tab}, groupId: ${groupId}`);
    
    // Call cleanup on the ImageGenerator instance if ref exists
    const imageGeneratorRef = imageGeneratorRefs.current[tab];
    if (imageGeneratorRef) {
      try {
        await imageGeneratorRef.cleanup();
        console.log(`[ImageGeneratorContainer] Successfully cleaned up tab ${tab}`);
      } catch (error) {
        console.error(`[ImageGeneratorContainer] Error cleaning up tab ${tab}:`, error);
      }
      
      // Remove ref
      delete imageGeneratorRefs.current[tab];
    }
  ...
  return (
    <>
      {/* 
        ...
        key={`image-gen-tab-${currentTab}`} forces React to completely unmount
        the old ImageGenerator instance and mount a fresh one when currentTab changes.
        This ensures complete state isolation between tabs with zero contamination.
      */}
      <ImageGenerator
        key={`image-gen-tab-${currentTab}`}
        ref={(ref) => {
          if (ref) {
            imageGeneratorRefs.current[currentTab] = ref;
          }
        }}
