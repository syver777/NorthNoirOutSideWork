...
// Mirrors supabase/functions/_shared/tokenCosts.ts.
...
function clampClipDurationForModel(model: string, requested: number): { duration: number; clamped: boolean } {
  ...
  if (cfg.durationOptions && cfg.durationOptions.length > 0) {
    ...
    // For slider-style models we honour any value in [min,max]; otherwise snap to closest option.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Token estimate: input=${estimate.inputTokens}, output=${estimate.outputTokens}, total=${estimatedTokens} (${validatedModel}, legacy=${isLegacyPlan})`);
    ...
    const triggerUrl = Deno.env.get('PROCESS_ITV_TASK_URL') || 'https://process-itv-task.storyscriptai.deno.net/';
    try {
