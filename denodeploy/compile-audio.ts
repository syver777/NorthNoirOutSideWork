...
const supabaseAnonKey = Deno.env.get('SUPABASE_PUBLIC_KEY') || '';

if (!supabaseUrl || !supabaseServiceRoleKey) {
  console.error('WARNING: SUPABASE_URL or SUPABASE_SECRET_KEY is not set');
}
...
async function triggerSizeCalculation(docId: string, filePath: string, version: number): Promise<void> {
  try {
    ...
    if (version === 9 || version === 10) {
      response = await fetch('https://audio-folder-size.storyscriptai.deno.net', {
        method: 'POST',
        headers: {
    ...
    if (videoProcess) {
      ...
      if (videoTask && videoTask.frequency_mode === 'audio') {
        ...
        try {
          ...
          const durationResponse = await fetch('https://calculate-audio-duration.storyscriptai.deno.net', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'apikey': supabaseServiceRoleKey,
          ...
          if (videoTask.image_prompt_status === 'pending' && videoTask.process_images !== false) {
            ...
            const setupPromptResponse = await fetch('https://storyscriptai-setup-prompt.storyscriptai.deno.net/', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'apikey': supabaseServiceRoleKey,
      ...
      if (videoTask && (visualType === 'ttv' || visualType === 'itv' || visualType === 'mg')) {
        try {
          ...
          if (videoTask.frequency_mode !== 'audio') {
            ...
            const durationResp = await fetch('https://calculate-audio-duration.storyscriptai.deno.net', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'apikey': supabaseServiceRoleKey,
          ...
          if (visualType === 'ttv' && videoTask.ttv_prompt_status === 'pending') {
            ...
            const ttvResponse = await fetch('https://setup-ttv-prompts.storyscriptai.deno.net', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'apikey': supabaseServiceRoleKey,
            ...
            const itvResponse = await fetch('https://setup-itv-prompts.storyscriptai.deno.net', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'apikey': supabaseServiceRoleKey,
            ...
            const mgResponse = await fetch('https://setup-mg-prompts.storyscriptai.deno.net', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'apikey': supabaseServiceRoleKey,
