...
const supabaseKey = Deno.env.get('SUPABASE_PUBLIC_KEY') ?? '';
const supabaseServiceRoleKey = Deno.env.get('SUPABASE_SECRET_KEY') ?? '';

if (!supabaseUrl || !supabaseKey) {
  throw new Error('Missing environment variables: SUPABASE_URL or SUPABASE_PUBLIC_KEY');
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (_allowedKeys.includes(_authToken)) {
      // Service call or backward-compatible frontend call (legacy or new keys)
    } else {
